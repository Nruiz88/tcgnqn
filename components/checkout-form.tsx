...
import { createOrder, createMercadoPagoOrder, quoteShippingForCheckout } from '@/lib/actions'
...
export default function CheckoutForm({
  ...
  const [method, setMethod] = useState<PaymentMethod['id']>(
    methods[0]?.id ?? 'transferencia',
  )
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    if (method === 'mercadopago') {
      const result = await createMercadoPagoOrder(formData)
      if (result?.error) {
            ...
            <div className="mt-1 space-y-2">
              {methods.map((m) => (
                ...
                >
                  ...
                  {m.id === 'mercadopago' && (
                    <span className="inline-flex items-center gap-1 rounded-full bg-[#009ee3]/10 px-2 py-0.5 text-xs font-semibold text-[#009ee3]">
                      <svg
                        className="h-3 w-3"
                        viewBox="0 0 24 24"
                        fill="none"
